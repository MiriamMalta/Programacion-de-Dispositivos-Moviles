...
class UserAuthRepository {
  ...
  Future<void> _createUserCollectionFirebase(User user) async {
    var userDoc =
      await FirebaseFirestore.instance.collection("p2_user").doc(user.uid).get();
    // Si no exite el doc, lo crea con valor default lista vacia
    if (!userDoc.exists) {
      await FirebaseFirestore.instance.collection("p2_user").doc(user.uid).set({
        'id': user.uid,
        'username': user.displayName,
        'profilePicture': user.photoURL,
        'email': user.email,
      });
      await FirebaseFirestore.instance.collection("p2_song").doc(user.uid).set(
        {
          'useruid': user.uid,
          'favorites': [],
        },
